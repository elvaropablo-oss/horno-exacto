...
(() => {
  ...
  const read = () => {
    try {
      ...
      return Array.isArray(value) ? value.filter((item) => item && item.id && item.path).slice(0, 100) : [];
    } catch { return []; }
  };
  const write = (items) => {
    try { localStorage.setItem(key, JSON.stringify(items.slice(0, 100))); return true; }
    catch { return false; }
  };
  const dateText = (iso) => {
  ...
  launcher.className = 'project-library-launcher';
  const dialog = document.createElement('dialog');
  dialog.className = 'project-library-dialog';
  dialog.setAttribute('aria-label', 'Mis proyectos guardados');
  const shell = document.createElement('div');
  shell.className = 'project-library-shell';
  const head = document.createElement('div');
  head.className = 'project-library-head';
  const heading = document.createElement('h2');
  heading.textContent = 'Mis proyectos';
  const close = document.createElement('button');
  close.type = 'button'; close.className = 'project-library-close'; close.setAttribute('aria-label', 'Cerrar'); close.textContent = '×';
  head.append(heading, close);
  const note = document.createElement('p');
  note.className = 'project-library-note';
  note.textContent = 'Se guardan únicamente en este navegador. No necesitas una cuenta.';
  const list = document.createElement('div');
  list.className = 'project-library-list';
  ...
  const render = () => {
    const items = read();
    launcher.textContent = 'Mis proyectos · ' + items.length;
    list.replaceChildren();
    if (!items.length) {
      const empty = document.createElement('p'); empty.className = 'project-library-empty'; empty.textContent = 'Todavía no has guardado ningún cálculo.'; list.append(empty); return;
    }
    for (const project of items) {
      ...
      open.addEventListener('click', () => { location.href = new URL(project.path, location.origin).href; });
      const duplicate = document.createElement('button'); duplicate.type = 'button'; duplicate.textContent = 'Duplicar';
      duplicate.addEventListener('click', () => {
        const current = read(); const source = current.find((item) => item.id === project.id); if (!source) return;
        current.unshift({ ...source, id: uid(), name: (source.name || 'Proyecto') + ' (copia)', createdAt: new Date().toISOString() }); write(current); render();
      });
      const remove = document.createElement('button'); remove.type = 'button'; remove.textContent = 'Eliminar';
      remove.addEventListener('click', () => { if (!confirm('¿Eliminar este proyecto guardado?')) return; write(read().filter((item) => item.id !== project.id)); render(); });
  ...
  const saveProject = (form, button) => {
    ...
    if (!url.searchParams.has('calc')) { alert('Haz primero un cálculo para poder guardarlo.'); return; }
    ...
    const entered = prompt('Nombre del proyecto', suggested);
    if (entered === null) return;
    const name = entered.trim() || suggested;
    const items = read();
    items.unshift({ id: uid(), name, page: pageTitle, formId: form.id, path: location.pathname + location.search + location.hash, createdAt: new Date().toISOString() });
    if (!write(items)) { alert('No se ha podido guardar el proyecto en este navegador.'); return; }
    render();
    const original = button.textContent; button.textContent = 'Proyecto guardado'; setTimeout(() => { button.textContent = original; }, 1600);
  };
  const attachSave = (form) => {
    const result = document.getElementById(form.id.replace(/-form$/, '-result'));
    ...
    const button = document.createElement('button'); button.type = 'button'; button.className = 'button button--quiet project-save-button'; button.dataset.saveProject = ''; button.textContent = 'Guardar en mis proyectos';
    button.addEventListener('click', () => saveProject(form, button)); result.append(button);
  };
  document.addEventListener('submit', (event) => {
    const form = event.target;
    if (!(form instanceof HTMLFormElement) || !allowed.has(form.id)) return;
    document.getElementById(form.id.replace(/-form$/, '-result'))?.querySelector('[data-save-project]')?.remove();
    setTimeout(() => attachSave(form), 0);
  }, true);
  render();
})();
  </script>`;
